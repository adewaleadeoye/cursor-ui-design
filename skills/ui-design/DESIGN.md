...
This file is the **fallback** when a repo has no `DESIGN.md`. Copy it into a project and replace every value. Do not treat these blues/neutrals as a brand.
...
Name each screen `marketing` | `app` | `data` | `form` | `media`. Density follows [standards.md](standards.md).
...
If this file has no freeze yet, take two shades or a gradient from the user and expand them per [color.md](color.md). Any family except slop kits. Do not default to these fallback blues.
